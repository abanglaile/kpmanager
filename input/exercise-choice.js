        ...
        const choiceRow = choice.map((item, i) => {
            return(
                <Row className="choice_row" type="flex" justify="space-between">
                    <Col span={10}>
                        <Checkbox checked={item.correct} onChange={(e) => this.onSelect(e, i)} />
                        <Input className="edit_choice_input" defaultValue={item.value} onChange={(e)=>this.handleChange(e, i)} rows={1} />
                    </Col>
                    <Col span={10}>
                        <Checkbox checked={item.correct} style={{ float: 'left', display: 'inline'}}> 
                            <div>checkbox</div>
                        <Checkbox/>
                        <Tex className="edit_choice_tex" content={item.value} />
                    </Col>
                </Row>
                );
        });
